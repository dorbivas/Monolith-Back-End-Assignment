...
class APIData:
    ...
    def get_attributes(self):
        # attr_dict = dict()
        # labels = set()
        # for attr in self.__data.attributes:
        #     for lbl in attr.labels:
        #         labels.add(lbl)
        # for lbl in labels:
        #     attr_dict.update({lbl: self.count_attributes(lbl)})
    ...
    # def count_attributes(self, label):
    #     res = 0
    ...
    def init_html(self):
        with open("Table.html", "w") as html_file:
            ...
            html_file.write(json2html.convert(json.dumps(self.get_products() , default=lambda o: o.__dict__,sort_keys=False, indent=2)))
            html_file.write("<h2>Attributes: </h2>")
            html_file.write(json2html.convert(json.dumps(self.get_attributes(), default=lambda o: o.__dict__, sort_keys=False, indent=2)))
            html_file.write("<h2>Categories: </h2>")
            html_file.write(json2html.convert(json.dumps(self.get_categories(), default=lambda o: o.__dict__, sort_keys=False, indent=2)))
